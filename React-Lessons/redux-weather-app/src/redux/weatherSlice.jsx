...
export const getWeatherCity = createAsyncThunk("weather/getWeatherCity", async (city) => {
    ...
    const current = await axios(`https://api.openweathermap.org/data/2.5/weather?lat=${val[0]}&lon=${val[1]}&appid=f67f5df8d7557f4e10221cb7a85b05a3&units=metric`);
    return current.data;
});
...
export const getForecastHourly = createAsyncThunk("weather/getForecastHourly", async (city) => {
    ...
    const hourly = await axios(`https://api.openweathermap.org/data/2.5/forecast?lat=${val[0]}&lon=${val[1]}&units=metric&cnt=10&appid=f67f5df8d7557f4e10221cb7a85b05a3`);
    return hourly.data;
});
...
export const getForecastDaily = createAsyncThunk("weather/getForecastDaily", async (city) => {
    ...
    const daily = await axios(`https://api.openweathermap.org/data/2.5/forecast?lat=${val[0]}&lon=${val[1]}&units=metric&appid=f67f5df8d7557f4e10221cb7a85b05a3`);
    const arr = [];
